...
const CoverLetterGenerator = () => {
    ...
    return (<section className="mt-6">
      ...
      <textarea placeholder="Generated cover letter will appear here..." className="border p-2 w-full mt-2" value={coverLetter} onChange={(e) => setCoverLetter(e.target.value)}></textarea>
      <div className="mt-2">
        <button className="bg-blue-500 text-white px-3 py-1" onClick={generateCoverLetter}>
          Generate Cover Letter
        </button>
        <button className="bg-green-500 text-white px-3 py-1 ml-2" onClick={downloadPDF}>
          Download PDF
        </button>
      </div>
    </section>);
