...
import { Box, Button, Container, Typography, TextField, List, ListItem, ListItemText, Stack, Divider } from '@mui/material';
const ResumeDashboard = () => {
    ...
    <Container maxWidth="md" sx={{ mt: 4, color: 'text.primary' }}>
      ...
      <TextField label="Paste job description here" multiline rows={4} value={jobDescription} onChange={(e) => setJobDescription(e.target.value)} fullWidth/>

      <Button variant="contained" color="success" sx={{ my: 2 }}>
        Enhance My Resume
      </Button>

      {aiSuggestions.length > 0 && (<Box>
          <Typography variant="h6" color="primary">AI Suggestions</Typography>
          <List>
            {aiSuggestions.map((suggestion, index) => (<ListItem key={index}>
      ...
      <TextField multiline rows={8} value={coverLetter} placeholder="Generated cover letter will appear here..." fullWidth InputProps={{ readOnly: true }}/>
